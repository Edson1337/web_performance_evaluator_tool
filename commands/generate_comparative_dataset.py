...
def generate_comparative_dataset():
    ...
    csr_df = csr_df.rename(columns={col: f"{col}_csr" for col in csr_df.columns if col not in ['project_name', 'route', 'scenario', 'iteration']})
    ssr_df = ssr_df.rename(columns={col: f"{col}_ssr" for col in ssr_df.columns if col not in ['project_name', 'route', 'scenario', 'iteration']})

    # Mesclar as colunas CSR e SSR
    combined_df = pd.merge(csr_df, ssr_df, on=['project_name', 'route', 'scenario', 'iteration'], how='outer')
    ...
    metrics = [col for col in combined_df.columns if col not in ['project_name', 'benchmark', 'scenario', 'iteration', 'route']]
    metrics_sorted = sorted(set([col.replace('_csr', '').replace('_ssr', '') for col in metrics]))
    columns_order = ['project_name', 'benchmark', 'scenario', 'iteration']
